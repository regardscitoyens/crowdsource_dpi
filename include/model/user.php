...
function users_top($limit = 5) {
  ...
  $req = $bdd->prepare("SELECT count(tasks.id) as nb, userid, users.nickname, twitter, website FROM users, tasks WHERE tasks.userid = users.id AND tasks.ip != 'VIRTUAL' GROUP BY users.id ORDER BY count(tasks.id) DESC LIMIT ".$limit);
  $req->execute();
  $top = array();
